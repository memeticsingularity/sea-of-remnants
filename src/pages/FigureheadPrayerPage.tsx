...
export function FigureheadPrayerPage() {
  ...
  const { state, toggleOwned, toggleFocusTag, clearOwned } = useGuardianState()

  const ownedCount = guardians.filter((g) => state.ownedIds.includes(g.id)).length
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <Card className="px-3 py-2">
            <span className="text-sm text-text-muted">已拥有守护</span>
            <span className="ml-2 text-lg font-bold text-accent">
              {ownedCount} / {guardians.length}
            </span>
          </Card>
          <Link
            to="/crews/figurehead-prayer/recommend"
